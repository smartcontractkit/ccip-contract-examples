...
func Abigen(a AbigenArgs) {
	...
	abigenExecutablePath := filepath.Join(GetProjectRoot(), "scripts/abigen")
	...
	// Add metadata to exported package only if metadata env var is set to "true"
	if os.Getenv("metadata") == "true" {
